...
public class Move extends CommandBase {
    ...
    public Move(
        ...
        this(hardware, north, east, rotation, hardware.getPosition(), hardware.getHeading());
    }
    ...
    public Move(
        ...
        this(hardware, north, east, 0, hardware.getPosition());
    }
    ...
    public Move(
        ...
        this(hardware, 0, 0, rotation, hardware.getHeading());
    }
    ...
    @Override public void initialize() {
        ...
        for(int i = 0; i < 3; i++) {
            ...
            fast[i] = Math.max(min.get(axis), max.get(axis) * scale[i]) * sign[i];// don't let maximum scale below minimum

            slow[i] = min.get(axis) * sign[i]; // don't scale below minimum

            distanceRemaining[i] = distanceTotal[i];
            finished[i] = Math.abs(distanceRemaining[i]) == 0.;
        }
    ...
    @Override public void execute() {
        ...
        Map<VelocityDirection, Double> distanceTraveled = (new Position(chassis.getPosition())).distanceRelative(origin);
        ...
        for(int i = 0; i < 3; i++) {
            ...
            double velocity = currentVelocity.get(axis);
            double distanceToStop = velocity * velocity / 2 / acceleration.get(axis, Type.DECELERATION);
    ...
    @Override public boolean isFinished() {
        for(int i = 0; i < 3; i++) {
            boolean done = finished[i];
            boolean noneToGo = distanceRemaining[i] == 0;
            boolean wentTooFar = Math.signum(distanceRemaining[i]) != Math.signum(distanceTotal[i]);

            finished[i] = done || noneToGo || wentTooFar;
        }
