 ...
 * <dl>
 *  <dt>{@link #targetsForVelocity(Map, Point)}</dt>
 ...
 *  <dt>{@link ChassisKinematics#velocityForTargets(Map)}}</dt>
 ...
 * call the simpler {@link ChassisKinematics#targetsForVelocity(Map)} and {@link ChassisKinematics#velocityForTargets(Map)} methods with the current robot motion (<b>[V]</b>)
...
public final class MecanumMath extends RectangularChassisKinematics<Double, Double> {
     ...
     * Calls to {@link ChassisKinematics#targetsForVelocity(Map)} (double[])} or {@link ChassisKinematics#velocityForTargets(Map)} will calculate translation motions
     ...
     *                            For example, if wheelSpeedFactor == wheelRadius, wheel velocities supplied to {@link ChassisKinematics#velocityForTargets(Map)} and
     *                            returned from {@link ChassisKinematics#targetsForVelocity(Map)} will be in user distance units.
     ...
     *                            For example, if rotationSpeedFactor = 180/Math.PI, robot rotations passed to {@link ChassisKinematics#targetsForVelocity(Map)}
     *                            and returned from {@link ChassisKinematics#velocityForTargets(Map)} will be in degrees.
     */
     ...
     * @param rotationSpeedFactor Multiplier to convert robot rotations in radians to user rotation units.
     *                            For example, if rotationSpeedFactor = 180/Math.PI, robot rotations passed to {@link ChassisKinematics#targetsForVelocity(Map)}
     *                            and returned from {@link ChassisKinematics#velocityForTargets(Map)} (double[])} will be in degrees.
     * @param rotationCenter      {N,E} coordinates of the center of rotation relative to the robot's geometric center.
     */
     ...
     * This method wraps the static {@link #targetsForVelocity(Map, Point)} method, passing the internal
     ...
     */
    @Override
    public Map<WheelPosition, Double> targetsForVelocity(Map<VelocityDirection, Double> translationMatrix) {
     ...
     * @return The wheel speed vector <b>[&Omega;]</b>: [NE, SE, SW, NW] in user units.
     */
    private Map<WheelPosition, Double> targetsForVelocity(Map<VelocityDirection, Double> translationMatrix, Point rotationCenter) {
        ...
        Map<WheelPosition, Double> wheelSpeedVector = new EnumMap<>(WheelPosition.class);
     ...
     */
    @Override
    public Map<VelocityDirection, Double> velocityForTargets(Map<WheelPosition, Double> wheelSpeeds) {
        Map<VelocityDirection, Double> translation = new EnumMap<>(VelocityDirection.class);
     ...
     * as input to {@link ChassisKinematics#velocityForTargets(Map)}.
     * @return Wheel rotational velicity in radians
     * as expected by {@link ChassisKinematics#velocityForTargets(Map)}.
     */
     ...
     * as input to {@link ChassisKinematics#targetsForVelocity(Map)}.
     * @return Velocity component in length distance
     * as expected by {@link #targetsForVelocity(Map, Point)}.
     */
     ...
     * as returned by {@link ChassisKinematics#targetsForVelocity(Map)}.
     * @return Speed in user units as returned by {@link ChassisKinematics#targetsForVelocity(Map)}.
     */
     ...
     * as returned by {@link ChassisKinematics#velocityForTargets(Map)}.
     * @return Speed in user units as returned by {@link ChassisKinematics#velocityForTargets(Map)}.
     */
     ...
     * as expected by {@link ChassisKinematics#targetsForVelocity(Map)}.
     */
